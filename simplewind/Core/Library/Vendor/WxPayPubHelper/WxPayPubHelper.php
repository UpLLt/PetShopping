...
class Wxpay_client_pub extends Common_util_pub
{
     ...
     *  作用：appapi调用微信支付所需参数
     */
    function getResultAppApi()
    {
        $this->getResult();
        if ($this->result['return_code'] == 'SUCCESS') {
            $this->appters['appid'] = $this->result['appid'];
            $this->appters['partnerid'] = $this->result['mch_id'];
            $this->appters['prepayid'] = $this->result['prepay_id'];
            $this->appters['package'] = "Sign=WXPay";
            $this->appters['noncestr'] = $this->createNoncestr();
            $this->appters['timestamp'] = time();
            $this->appters['sign'] = $this->getSign($this->appters);


            $this->appters['err_code'] = $this->result['err_code_des'];
            unset($this->appters['package']);
            $this->appters['packages'] = "Sign=WXPay";
            return $this->appters;
        } else {
            return false;
        }
...
class UnifiedOrder_pub extends Wxpay_client_pub
{
     ...
     * 获取prepay_id
     */
    function getPrepayId($dump = false)
    {
        $this->postXml();
        $this->result = $this->xmlToArray($this->response);
...
class JsApi_pub extends Common_util_pub
{
     ...
     * @return string
     */
    function createOauthUrlForUserInfo($access_token, $openid)
    {
